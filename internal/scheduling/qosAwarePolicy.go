...
func (p *QosAwarePolicy) handleHAReq(r *scheduledRequest) {
...
func (p *QosAwarePolicy) handleHighPerfReq(r *scheduledRequest) {
	var timeLocal, timeOffload float64
	logger := logging.GetLogger()
	localStatus, _ := logger.GetLocalLogStatus(r.Fun.Name)
	remoteStatus, _ := logger.GetRemoteLogStatus(r.Fun.Name)
...
func (p *QosAwarePolicy) handleLowReq(r *scheduledRequest) {
	logger := logging.GetLogger()
	remoteStatus, _ := logger.GetRemoteLogStatus(r.Fun.Name)
